...
1. They both require a [trusted setup](../../terms/trusted_setup.md). For [SRS](../../terms/structured_reference_string.md), it is
   universal and updatable for Constant-round polynomial IOPs, computation-specific for Linear PCPs.
...
**Post-quantum security**: The approaches that are plausibly post-quantum secure are comprised of those
...
**Dominant contributor to cost - polynomial commitments:** Normally, the polynomial commitment dominates the most relevant costs:
prover time, proof length, and verifier time when combined with
MIPs and constant-round polynomial IOPs.

> [!NOTE]
> There is an exception that, if an MIP is combined with KZG commitments, it is the MIP and not the polynomial commitment that
> dominates
> verification costs.
